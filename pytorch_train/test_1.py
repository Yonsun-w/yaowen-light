...
from test_2 import Cal_params_epoch, Model_eval
from generator import getTimePeriod
import datetime

def selectModel(NetName):
    if NetName == 'ADSNet':
...
def DoTrain(config_dict):
    ...
    # model
    model = selectModel('MOE')

    ModelFilePath = '/home/wenjiahua/windows_project/pre/models/moe/_LightNet_model_maxETS.pkl'

    model_file = torch.load(ModelFilePath, map_location=torch.device(config_dict['Device']))
    ...
    model_eval_valdata.eval(val_loader, model, 0)
