...
class TxtTrueFiletoGrid(object):
    def __init__(self, config_dict):
        ...
        self.lon_gap = float(config_dict['lonGap'])
        # n代表纬度上的
        self.lat_gap = float(config_dict['latGap'])
    ...
    def create_nc(self, output_path, txt_path, time):
        ...
        # 这里等经纬度 等距离不知道怎么写
        if config['equal_dis'] == 1:
            f_w.delta_lat = config['lonGap']
        else :
            f_w.delta_dis = config['lonGap'] + '' + 'km'
        # 这里设置的是时间分辨率
        ...
        with open(tFilePath, 'r', encoding='GBK') as tfile:
            for line in tfile:
                ...
                if self.config_dict['equal_dis'] == 0:
                    x = int((self.lat_max - self.lat_min) / self.lat_gap)
                    y = int((self.lon_max - self.lon_min) / self.lon_gap)
